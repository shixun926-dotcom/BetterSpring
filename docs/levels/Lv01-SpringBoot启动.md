...
> 下面是本关的**答案**。**先自己做。**
>
> 允许查看的门槛（[00-维护规范 §10 参考解使用规则](../00-文档使用与维护规范.md#10-参考解使用规则)）：
> 本关 §6 的「故意制造错误」实验至少亲手做过一遍，**或者**在同一处卡住超过 60 分钟。
>
> **看过之后必须关掉它，从空文件重敲一遍** —— 只有这样才算满足 `Lv01-A06`（"代码是自己敲的，不是复制的"）。

| 项 | 位置 |
| --- | --- |
| 本关参考解目录 | [`solutions/Lv01/`](../../solutions/Lv01/) |
| 本关做了什么 / 怎么验收 / 实验对照 / 练习答案 | [`solutions/Lv01/README.md`](../../solutions/Lv01/README.md) |
| 本关增量文件（9 个） | [`solutions/Lv01/campus/`](../../solutions/Lv01/campus/) —— 路径与工程根的 `campus/` **一一对应** |
| 完整可运行快照 | `git checkout lv01-pass` |

📌 **`solutions/Lv01/` 一经写入即冻结**，不随 `campus/` 后续演进修改（[`SOL-004`](../00-文档使用与维护规范.md#10-参考解使用规则)）。
Lv01 是起点，所以本关"增量"就是工程骨架的全部 —— 此刻它与 `campus/` 内容完全相同，**从 Lv02 起两者开始分叉**。
